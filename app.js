...
window.inviaTelegram = function () {
    ...
    let num = telClienteAttuale.replace(/[^0-9]/g, '');
    if (num.length <= 10 && !num.startsWith('39')) num = '39' + num;
    window.open(`whatsapp://send?phone=${num}?text=${encodeURIComponent(msgDaInviarePlain)}`, '_blank');
};
...
window.gestisciImportazioneCSV = function (event, tabella) {
    ...
    reader.onload = async function (e) {
        ...
        if (tabella === 'vendite') {
            ...
            let timeBase = Date.now(); // Generatore di ID base

            if (venditeDaSalvare.length > 0) {
                let tx = db.transaction('vendite', 'readwrite');
                let store = tx.objectStore('vendite');
                venditeDaSalvare.forEach((v, index) => {
                    if (!v.id) v.id = timeBase + index; // Assegna un ID univoco
                    store.add(v);

                    // 🔥 SINCRONIZZAZIONE CLOUD VENDITE
            ...
            if (movimentiDaSalvare.length > 0) {
                ...
                let timeBaseMov = Date.now() + 500000; // Sfalsa gli ID per sicurezza
                movimentiDaSalvare.forEach((m, index) => {
                    if (!m.id) m.id = timeBaseMov + index; // Assegna un ID univoco
                    storeMov.add(m);

                    // 🔥 SINCRONIZZAZIONE CLOUD MOVIMENTI (Spese/Entrate)
...
window.filtraDashboardGiftCard = function () {
    ...
    filtrati.forEach(gc => {
        ...
        // 🔥 NOVITÀ: Tasto dinamico per le Sopravvenienze Attive
        let btnAzione = `<button class="btn-modal btn-blu" style="padding: 4px 10px; margin: 0; font-size: 1.2vh;" onclick="verificaRapidaGiftCard('${gc.codice}')">🔍 GESTISCI</button>`;

        if (statoReale === 'SCADUTA_CREDITO') {
            // Se è scaduta ma ha soldi, cambiamo il tasto!
            btnAzione = `<button class="btn-modal btn-rosso" style="padding: 4px 10px; margin: 0; font-size: 1.1vh; border-color:#ff4d4d;" onclick="contabilizzaSopravvenienzaGC('${gc.codice}', ${gc.saldo})">💰 INCASSA</button>`;
        }
        ...
        tr.innerHTML = `
            ...
            <td style="padding: 8px; text-align: center;">${btnAzione}</td>
        `;
        tbody.appendChild(tr);
    });
};
